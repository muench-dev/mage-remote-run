...
async function configureAndTestConnection(name, initialSettings = {}) {
    ...
    settings = await updateB2BModuleAvailability(settings, lastTestError);
    return settings;
}
...
async function updateB2BModuleAvailability(settings, lastTestError) {
    ...
    if (!shouldCheckB2BModules(settings)) {
        if (settings && 'b2bModulesAvailable' in settings) {
            delete settings.b2bModulesAvailable;
        }
        return settings;
    }

    try {
        settings.b2bModulesAvailable = await detectB2BModules(settings);
        ...
        if (process.env.DEBUG) {
            ...
            console.log(chalk.gray(`DEBUG: Unable to detect B2B modules${suffix}: ${e.message}`));
        }
        settings.b2bModulesAvailable = null;
    }

    return settings;
}

async function detectB2BModules(settings) {
    const client = await createClient(settings);
...
async function ensureB2BStatus(profileName, profile, config) {
    ...
    if (!shouldCheckB2BModules(profile)) {
        return false;
    }

    try {
        profile.b2bModulesAvailable = await detectB2BModules(profile);
    } catch (e) {
        profile.b2bModulesAvailable = null;
    }
...
`)
        ...
        if (['ac-cloud-paas', 'ac-on-prem', 'ac-saas'].includes(profile.type)) {
            ...
            console.log(`B2B Modules: ${b2bStatus}`);
        }
    } else {
        ...
        .action(async () => {
            try {
                ...
                for (const [name, profile] of Object.entries(config.profiles || {})) {
                    updated = (await ensureB2BStatus(name, profile, config)) || updated;
                }
                if (updated) {
                    await saveConfig(config);
                }
                ...
                printTable(['Name', 'Type', 'URL', 'B2B', 'Active'], rows);
...
`)
        .action(async () => {
            try {
                ...
                if (activeProfile && await ensureB2BStatus(activeProfileName, activeProfile, config)) {
                    await saveConfig(config);
                }
