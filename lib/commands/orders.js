...
export function registerOrdersCommands(program) {
  ...
  const listCommand = addFormatOption(addPaginationOptions(orders.command('list')))
...
`);
  ...
  addSortOption(addFilterOption(listCommand)).action(listOrdersAction);
...
`)
  ...
  addFormatOption(orders.command('show <identifier>')
...
`)
  ...
  addPaginationOptions(orders.command('latest')
    .description('List latest orders sorted by created_at DESC with selection'))
    .option('--select', 'Interactive selection mode')
    .addHelpText('after', `
Examples:
