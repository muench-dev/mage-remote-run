...
import { printTable, handleError, buildSearchCriteria } from '../utils.js';
    ...
    try {
        ...
        if (options.sort && options.sort.length > 0) {
            for (const s of options.sort) {
                ...
                params[`searchCriteria[sortOrders][${sortIndex}][direction]`] = dir;
                sortIndex++;
            }
        } else if (options.sortBy) {
            params[`searchCriteria[sortOrders][${sortIndex}][field]`] = options.sortBy;
            params[`searchCriteria[sortOrders][${sortIndex}][direction]`] = options.sortOrder || 'ASC';
        }
