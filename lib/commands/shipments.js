...
export function registerShipmentCommands(program) {
  ...
  addFormatOption(addSortOption(addFilterOption(addPaginationOptions(shipments.command('list')
...
Examples:
  ...
  $ mage-remote-run shipment list --filter "order_id=123"
  $ mage-remote-run shipment list --sort "created_at:DESC"
`)
    .action(listShipmentsAction);

  addFormatOption(shipments.command('show <id>')
