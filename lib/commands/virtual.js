...
import { handleError, formatOutput, addFormatOption, addFilterOption, addSortOption, addPaginationOptions, buildSearchCriteria, buildSortCriteria, isInteractiveMode } from '../utils.js';
...
function addCommandOption(command, flags, description, defaultValue, required = false, choices = null) {
    if (hasOption(command, flags)) {
        return command;
    }

    const validChoices = Array.isArray(choices) && choices.length > 0 ? choices : null;

    if (validChoices) {
        const choiceValues = validChoices.map(c => typeof c === 'string' ? c : String(c.value));
        const opt = new Option(flags, description).choices(choiceValues);
        if (defaultValue !== undefined) {
            opt.default(defaultValue);
        }
        return command.addOption(opt);
    }
...
const SENSITIVE_FIELD = /password|passwd|passphrase|secret|token|apikey|api[_-]key|credential/i;

function isSensitiveOption(name, description = '') {
    return SENSITIVE_FIELD.test(name) || SENSITIVE_FIELD.test(description);
}

const PURE_DOLLAR_PLACEHOLDER = /^\$\{([a-zA-Z0-9_]+)\}$/;
const PURE_BRACE_PLACEHOLDER  = /^\{:\s*([a-zA-Z0-9_]+)\s*\}$/;

function interpolateBodyTemplate(template, values) {
    if (template === null || template === undefined) return template;

    if (typeof template === 'string') {
        const dm = PURE_DOLLAR_PLACEHOLDER.exec(template);
        if (dm) {
            const v = values[dm[1]];
            return v !== undefined && v !== null ? v : template;
        }
        const bm = PURE_BRACE_PLACEHOLDER.exec(template);
        if (bm) {
            const v = values[bm[1]];
            return v !== undefined && v !== null ? v : template;
        }
        return interpolateTemplateValue(template, values);
    }

    if (Array.isArray(template)) {
        return template.map(item => interpolateBodyTemplate(item, values));
    }

    if (typeof template === 'object') {
        const result = {};
        for (const [k, v] of Object.entries(template)) {
            result[k] = interpolateBodyTemplate(v, values);
        }
        return result;
    }
...
export function registerVirtualCommands(program, config, profile) {
    ...
    for (const cmdDef of config.commands) {
        ...
        for (const key of parameterKeys) {
            ...
            addCommandOption(currentCmd, flags, desc, paramDef.default, paramDef.required, paramDef.choices);
        }
        ...
        currentCmd.action(async (options) => {
            try {
                ...
                // Resolve missing option values — prompt interactively or collect errors
                ...
                for (const key of parameterKeys) {
                    ...
                    if (paramDef.type === 'boolean') continue;

                    const choices = Array.isArray(paramDef.choices) && paramDef.choices.length > 0 ? paramDef.choices : null;
                    const isRequired = !!paramDef.required;

                    if (!interactive) {
                        if (isRequired) missingRequired.push(key);
                        continue;
                    }

                    if (choices) {
                        const choiceItems = choices.map(c => typeof c === 'string' ? { name: c, value: c } : c);
                        options[key] = await select({
                            message: paramDef.description || `Select ${key}:`,
                            choices: choiceItems,
                            default: paramDef.default
                        });
                    } else if (isRequired) {
                        const message = paramDef.description ? `${paramDef.description}:` : `Enter ${key}:`;
                        if (isSensitiveOption(key, paramDef.description)) {
                            options[key] = await password({
                                message,
                                validate: value => value.length > 0 || `${key} is required`
                            });
                        } else {
                ...
                if (missingRequired.length > 0) {
                    const flags = missingRequired.map(k => `--${k}`).join(', ');
                    const label = missingRequired.length === 1 ? 'option' : 'options';
                    throw new Error(`Missing required ${label}: ${flags}`);
                }

                let queryParams = {};

                // Step 1: substitute path parameters and track which keys were used
                ...
                let payload;
                if (cmdDef.body !== null && cmdDef.body !== undefined && typeof cmdDef.body === 'object') {
                    payload = interpolateBodyTemplate(cmdDef.body, options);
                } else {
                    payload = {};
                    for (const key of parameterKeys) {
                ...
                const resolvedPayload = (() => {
                    ...
                    if (!Array.isArray(payload) && typeof payload === 'object' && Object.keys(payload).length === 0) return undefined;
                    return payload;
                })();
