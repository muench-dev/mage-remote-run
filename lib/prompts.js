...
export async function askForProfileSettings(defaults = {}) {
    ...
    if (type === 'ac-saas') {
        ...
        const tokenUrl = await input({
            message: 'IMS Token URL (Optional):',
            default: defaults.auth?.tokenUrl || 'https://ims-na1.adobelogin.com/ims/token/v3'
        });
        ...
        authConfig = {
            clientId,
            clientSecret: clientSecret || defaults.auth?.clientSecret,
            tokenUrl
        };
    } else {
