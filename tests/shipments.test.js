...
describe('Shipment Commands', () => {
    ...
    test('list shipments with json format', async () => {
        registerShipmentCommands(program);
        mockClient.get.mockResolvedValue({
            items: [{ entity_id: 1, increment_id: '001' }],
            total_count: 1
        });

        await program.parseAsync(['node', 'test', 'shipment', 'list', '--format', 'json']);

        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('"items"'));
    });

    test('show shipment with items, tracks and comments', async () => {
        registerShipmentCommands(program);
        mockClient.get.mockResolvedValue({
            entity_id: 1,
            increment_id: '001',
            ...
            items: [{ sku: 'SKU1', name: 'Product 1', qty: 1, price: 10 }],
            tracks: [{ title: 'FedEx', carrier_code: 'fedex', track_number: '12345' }],
            comments: [{ created_at: '2023-01-01', comment: 'Test comment' }]
        });

        await program.parseAsync(['node', 'test', 'shipment', 'show', '1']);

        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Shipment Information'));
        expect(printTable).toHaveBeenCalledTimes(3);
    });

    test('show shipment with json format', async () => {
        registerShipmentCommands(program);
        mockClient.get.mockResolvedValue({ entity_id: 1, increment_id: '001' });

        await program.parseAsync(['node', 'test', 'shipment', 'show', '1', '--format', 'json']);

        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('"entity_id"'));
    });

    test('show shipment fallback to search by increment_id', async () => {
        registerShipmentCommands(program);
        mockClient.get
            .mockRejectedValueOnce(Object.assign(new Error('Not Found'), { response: { status: 404 } }))
            .mockResolvedValueOnce({
                items: [{ entity_id: 5, increment_id: '00000001', order_id: 10, total_qty: 1, created_at: '2023-01-01', items: [], tracks: [], comments: [] }]
            });

        await program.parseAsync(['node', 'test', 'shipment', 'show', '00000001']);

        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Shipment Information'));
    });

    test('show shipment throws when not found via fallback', async () => {
        registerShipmentCommands(program);
        ...
        await program.parseAsync(['node', 'test', 'shipment', 'show', '99999']);

        expect(handleError).toHaveBeenCalled();
    });

    test('show shipment rethrows non-404 errors', async () => {
        registerShipmentCommands(program);
    ...
    test('label: should print label when found', async () => {
        registerShipmentCommands(program);
        mockClient.get.mockResolvedValue('base64labeldata');

        await program.parseAsync(['node', 'test', 'shipment', 'label', '100']);

        expect(mockClient.get).toHaveBeenCalledWith('V1/shipment/100/label');
        expect(consoleSpy).toHaveBeenCalledWith('base64labeldata');
    });

    test('label: should print no label message when not found', async () => {
        registerShipmentCommands(program);
    ...
    test('send email: should handle false result', async () => {
        registerShipmentCommands(program);
        mockClient.post.mockResolvedValue(false);
